   ...
   glDisable(GL_LIGHTING);
        ...
        glBegin(GL_POINTS);
            for(int j = 0; j<grupos.size(); j++){
                if(!grupos[j].verticesGrupo.empty()){
                for(int i = 0; i<grupos[j].verticesGrupo.size(); i++)
                    glVertex2d(grupos[j].verticesGrupo[i].z, grupos[j].verticesGrupo[i].y);
                }
            }
        ...
        glPushMatrix();
            ...
            glBegin(GL_TRIANGLES);
                for(int z=0 ; z<grupos.size(); z++){
                  ...
                  if(!grupos[z].verticesGrupo.empty() && !grupos[z].triangulosGrupo.empty()){
                        for(int i=0; i<grupos[z].triangulosGrupo.size(); i++){
                            vertice vetorNormal;
                            CalculaNormal(grupos[z].triangulosGrupo[i], &vetorNormal); // Passa face triangular e endereço do vetor normal de saída
                            glNormal3f(vetorNormal.x, vetorNormal.y,vetorNormal.z);
                            for(int j=0; j<3; j++)
                              glVertex3d(grupos[z].triangulosGrupo[i].v[j].x, grupos[z].triangulosGrupo[i].v[j].y, grupos[z].triangulosGrupo[i].v[j].z);
